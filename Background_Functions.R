...
Decomp2 <- function(output){
  io <- apply(do.call(rbind, # tria
                      lapply(output, sapply, lapply, function(q) q@fitmeasures)), #dyo 
              2, rbind) #ena
  o <- list()
  o$trueHF$fitHF <- as.data.frame(do.call(rbind, io$HF[,seq(1, 3000-2, 3)])) 
  o$trueHF$fitBF <- as.data.frame(do.call(rbind, io$HF[,seq(2, 3000-1, 3)]))
  o$trueHF$fitNW <- as.data.frame(do.call(rbind, io$HF[,seq(3, 3000  , 3)]))
  o$trueBF$fitHF <- as.data.frame(do.call(rbind, io$BF[,seq(1, 3000-2, 3)]))
  o$trueBF$fitBF <- as.data.frame(do.call(rbind, io$BF[,seq(2, 3000-1, 3)]))
  o$trueBF$fitNW <- as.data.frame(do.call(rbind, io$BF[,seq(3, 3000  , 3)]))
  o$trueNW$fitHF <- as.data.frame(do.call(rbind, io$NW[,seq(1, 3000-2, 3)]))
  o$trueNW$fitBF <- as.data.frame(do.call(rbind, io$NW[,seq(2, 3000-1, 3)]))
  o$trueNW$fitNW <- as.data.frame(do.call(rbind, io$NW[,seq(3, 3000  , 3)]))
