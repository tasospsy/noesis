...
Decomp <- function(simout, fitMeasures){
  ...
  stat <- list()
  # For True Model = HF
  stat$trueHF$fitHF <- tmp[seq(1, 3*nrep - 2, by = 3), seq(1, length(tmp) - 2, by = 3)]
  stat$trueHF$fitBF <- tmp[seq(2, 3*nrep - 1, by = 3), seq(1, length(tmp) - 2, by = 3)]
  stat$trueHF$fitNW <- tmp[seq(3, 3*nrep    , by = 3), seq(1, length(tmp) - 2, by = 3)]
  # For True Model = BF
  stat$trueBF$fitHF <- tmp[seq(1, 3*nrep - 2, by = 3), seq(2, length(tmp) - 1, by = 3)]
  stat$trueBF$fitBF <- tmp[seq(2, 3*nrep - 1, by = 3), seq(2, length(tmp) - 1, by = 3)]
  stat$trueBF$fitNW <- tmp[seq(3, 3*nrep    , by = 3), seq(2, length(tmp) - 1, by = 3)]
  # For True Model = NW
  stat$trueNW$fitHF <- tmp[seq(1, 3*nrep - 2, by = 3), seq(3, length(tmp)    , by = 3)]
  stat$trueNW$fitBF <- tmp[seq(2, 3*nrep - 1, by = 3), seq(3, length(tmp)    , by = 3)]
  stat$trueNW$fitNW <- tmp[seq(3, 3*nrep    , by = 3), seq(3, length(tmp)    , by = 3)]
